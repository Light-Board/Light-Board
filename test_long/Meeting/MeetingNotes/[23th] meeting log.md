...
2. 유저 AUTH (인증) / spring security **[공동 진행 중 - 전세영 완]**
    - 보통 스프링에서는 어떻게 하나?
    - 필터 / 토큰 / 분석
    ...
    - 실제로 도움된 링크 : https://blog.naver.com/PostView.nhn?isHttpsRedirect=true&blogId=kbh3983&logNo=220932183498&redirect=Dlog&widgetTypeCall=true


## 다음 회의 토픽

0. 알림 처리를 위한 kafka 세팅 시작
    - 브로커 서버 필요해요
    - (Pub) message 를 post request - (BROKER) - (Sub) message get하는 request
    - 각 메시지 형태당 API가 2개씩이긴 한데 설계하기 나름
    - 알림성 메시지 -> pub / BE(server) -> topic sub -> "message" [notionfication] model -> mongodb 에 저장
    - FE -> long polling -> mongodb get
...
- [ ] 알림 kafka 활용 방안에 5분 PT 준비, ppt 1장
- [ ] 회사 리쿠르팅 정보 전달 [정현우]